...
def select_csv_files():
    ...
    file_paths = filedialog.askopenfilenames(
        title="Select CSV Files to Merge", filetypes=[("CSV Files", "*.csv")]
    )

    if len(file_paths) < 2:
        messagebox.showerror("Error", "Please select at least two CSV files for merging.")
        return

    merge_columns = simpledialog.askstring("Input", "Enter the column name to merge on:")

    if merge_columns is None or merge_columns.strip() == "":
        messagebox.showerror("Error", "Please specify a column to merge on.")
        return

    directory = os.path.dirname(file_paths[0])

    output_filename = merge_csv_files(directory, [merge_columns])

    if output_filename:
        messagebox.showinfo("Success", f"Merged data saved to {output_filename}")
